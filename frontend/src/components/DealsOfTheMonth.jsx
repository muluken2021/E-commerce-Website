...
const DealsOfTheMonth = () => {
  ...
  // Countdown timer state (Days, Hr, Mins, Sec)
  ...
  const handlePrev = () => {
    setActiveIndex((prev) => (prev === 0 ? dealsData.length - 1 : prev - 1));
  };

  const handleNext = () => {
    setActiveIndex((prev) => (prev === dealsData.length - 1 ? 0 : prev + 1));
  };

  const formatNumber = (num) => String(num).padStart(2, '0');

  return (
    <section className="w-full bg-white py-16 px-6 md:px-12 max-w-7xl mx-auto overflow-hidden">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
        ...
        <div className="lg:col-span-5 flex flex-col items-start pr-0 lg:pr-6">
          ...
          <button className="bg-black text-white text-xs font-semibold px-8 py-3.5 rounded-lg shadow-md hover:bg-gray-800 transition-colors mb-10">
            Buy Now
          </button>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="w-10 h-10 rounded-full border border-gray-200 bg-white flex items-center justify-center text-gray-600 hover:bg-gray-50 shadow-sm transition-colors"
            >
              <ChevronLeft size={18} />
            </button>
            <button
        ...
        <div className="lg:col-span-7 flex flex-col items-center">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 w-full">
            {dealsData.slice(0, 3).map((item, index) => (
              <div
                key={item.id}
                className="relative h-[380px] rounded-xl overflow-hidden bg-gray-100 group shadow-sm"
              >
                <img
                  src={item.image}
                  alt={item.alt}
                ...
                {index === 0 && (
                  <div className="absolute bottom-6 left-6 bg-white py-3 px-4 rounded-md shadow-md">
            ...
            {dealsData.map((_, idx) => (
              <button
                ...
                className={`h-2 rounded-full transition-all duration-300 ${
                  activeIndex === idx ? 'w-5 bg-black' : 'w-2 bg-gray-300'
                }`}
              />
            ))}
          </div>
